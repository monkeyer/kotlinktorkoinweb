package fan.zheyuan.domain.repository
...
class BeerRepository : KoinComponent {
    ...
    fun createTable() {
        transaction { if (!Beers.exists()) SchemaUtils.create(Beers) }
    }
    ...
    fun findById(beerId: Int): Beer? {
        var beer: Beer? = null
        transaction {
            val res = Beers.select { Beers.sequenceId.eq(beerId) }.firstOrNull()
...
object Beers : IntIdTable() {
    val sequenceId = integer("sequel_id").autoIncrement().primaryKey()
